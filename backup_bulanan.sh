...
backupdir=/backup_db/bulanan/`date +\%y\%m\%d`
tanggal=`date +\%y\%m\%d`
#dir=`ls $backupdir`
...
if [ $RC -gt 0 ]; then
   ...
   swaks --to tony.arianto@klola.id --from "admin@klola.id" --header "Subject: Backup Database Bulanan Telah Berhasil" --body "Backup Database Bulanan Telah Berhasil" --server smtp.mandrillapp.com --port 587 --auth LOGIN --auth-user "tony.arianto@klola.id" --auth-password "Y7fJNofYlCL7Z8Z--vIkKg" -tls
fi
exit 0
